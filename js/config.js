// ── Config ──
    ...
    function loadCustomMeta() {
      try {
        ...
        if (raw) { const arr = JSON.parse(raw); if (Array.isArray(arr) && arr.length) CATS = arr; }
      } catch (e) {}
      try {
        const raw = localStorage.getItem(LS_CUSTOM_SUBCATS);
        if (raw) { const arr = JSON.parse(raw); if (Array.isArray(arr) && arr.length) SUBCATS = arr; }
      } catch (e) {}
      try {
        const raw = localStorage.getItem(LS_CUSTOM_FORMATOS);
        if (raw) { const arr = JSON.parse(raw); if (Array.isArray(arr) && arr.length) FORMATO_LIST = arr; }
      } catch (e) {}
      try {
        const raw = localStorage.getItem(LS_CUSTOM_STATUS);
        if (raw) { const arr = JSON.parse(raw); if (Array.isArray(arr) && arr.length) STATUS_LIST = arr; }
      } catch (e) {}
      try {
        const raw = localStorage.getItem(LS_CUSTOM_AIS);
        if (raw) { const arr = JSON.parse(raw); if (Array.isArray(arr) && arr.length) AI_LIST = arr; }
      } catch (e) {}
      rebuildLabelMaps();
      rebuildEditableCats();
    }
